    ...
    public static class VisibleEdgesTriangulationAlgorithm
    {
        ...
        private static HashSet<Triangle2> TriangulatePointsConvexHull(HashSet<MyVector2> points)
        {
            ...
            for (int i = 0; i < sortedPoints.Count; i++)
            {
                ...
                LeftOnRight pointRelation = GeometryUtility.IsPoint_Left_On_Right_OfVector(p1, p2, sortedPoints[i]);
            ...
            List<MyVector2> pointsOnHull = ConvexHull.JarvisMarch_2D(triangulatePoints);
            ...
            foreach (MyVector2 pointToAdd in sortedPoints)
            {
                ...
                for (int j = 0; j < pointsOnHull.Count; j++)
                {
                    ...
                    LeftOnRight pointRelation = GeometryUtility.IsPoint_Left_On_Right_OfVector(hull_p1, hull_p2, pointToAdd);
                    ...
                    if (GeometryUtility.IsTriangleOrientedClockwise(hull_p1, hull_p2, pointToAdd))
                    {                    
                ...
                if (couldFormTriangle)
                {
                    ...
                    pointsOnHull = ConvexHull.JarvisMarch_2D(new HashSet<MyVector2>(pointsOnHull));
                }
                else
                {
