...
namespace Habrador_Computational_Geometry
{
    ...
    public static class TriangleSplittingAlgorithm
    {
        public static HashSet<Triangle2> TriangulatePoints(HashSet<MyVector2> points, bool addColinearPoints)
        {
            ...
            List<MyVector2> pointsOnConvexHull = ConvexHull.JarvisMarch_2D(points);


            //Step 2. Triangulate the convex hull
            HashSet<Triangle2> triangles = Habrador_Computational_Geometry.TriangulatePoints.PointsOnConvexHull(pointsOnConvexHull, addColinearPoints: true);
            ...
            foreach (MyVector2 currentPoint in points)
            {
                ...
                {
                    if (GeometryUtility.PointTriangle(t, currentPoint, includeBorder: true))
                    {
