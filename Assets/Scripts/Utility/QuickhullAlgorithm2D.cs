...
{
    public class QuickhullAlgorithm2D
    {
        ...
        public static List<MyVector2> GenerateConvexHull(List<MyVector2> originalPoints, bool includeColinearPoints)
        {
            ...
            float maxDistanceSqr = float.NegativeInfinity;
            ...
            if (GeometryUtility.IsTriangleOrientedClockwise(tStart.p1, tStart.p2, tStart.p3))
            {
            ...
            {
                //p1 p2
                LeftOnRight pointRelation1 = GeometryUtility.IsPoint_Left_On_Right_OfVector(p1, p2, p);
                ...
                //p2 p3
                LeftOnRight pointRelation2 = GeometryUtility.IsPoint_Left_On_Right_OfVector(p2, p3, p);
        ...
        private static List<MyVector2> AddColinearPoints(List<MyVector2> pointsOnConvexHull, List<MyVector2> points)
        {
            ...
            for (int i = 0; i < pointsOnConvexHull.Count; i++)
            {
                ...
                {
                    LeftOnRight pointRelation = GeometryUtility.IsPoint_Left_On_Right_OfVector(p1, p2, p);
        ...
        private static void RemovePointsWithinTriangle(Triangle2 t, HashSet<MyVector2> points)
        {
            ...
            foreach (MyVector2 p in points)
            {
                if (GeometryUtility.PointTriangle(t, p, includeBorder: true))
                {
        ...
        private static MyVector2 FindPointFurthestFromEdge(MyVector2 p1, MyVector2 p2, HashSet<MyVector2> points)
        {
            ...
            foreach (MyVector2 p in points)
            {
                ...
                float distanceToEdge = GeometryUtility.GetSignedDistanceFromPointToPlane(p, plane);
